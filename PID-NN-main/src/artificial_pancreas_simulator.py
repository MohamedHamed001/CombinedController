...
class InsulinSimulator:
    ...
    def adjust_basal_insulin(self, glucose, Kp, Ki, Kd, timestep):
        ...
        pid_adjustment = (Kp * error) + (Ki * self.state["cumulative_error"]) + (Kd * derivative)
        
        # Calculate final rate, ensuring we don't go below minimum basal rate
        adjusted_rate = max(0.1, basal_dose + pid_adjustment)

        # Apply upper limit to the adjusted rate
        adjusted_rate = min(adjusted_rate, 1.5)
        ...
        print(f"Error: {error:.2f} mg/dL")
        print(f"PID Adjustment: {pid_adjustment:.2f} U/hr")
        print(f"Base Basal Rate: {basal_dose:.2f} U/hr")
        print(f"Adjusted Hourly Basal Insulin: {adjusted_rate:.2f} U/hr")
    ...
    def run(self, glucose, currentTimeInNs):
        ...
        if timestep <= self.sim_time:
            ...
            # If we're not giving bolus or basal, calculate and return the current basal rate
            else:
                rate = self.predict_insulin_dosage(glucose, timestep)
                self.state["basal_rates"].append(float(rate))
                self.state["infusion_values"].append(float(rate))
